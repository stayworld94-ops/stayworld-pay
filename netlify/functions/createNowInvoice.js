...
export async function handler(event){
  try{
    ...
    if(!apiKey) return { statusCode:500, body: JSON.stringify({ ok:false, error:'NOWPAYMENTS_API_KEY missing' }) };
    ...
    const payload = {
      ...
      success_url: `${site}/public/confirmation.html?provider=nowpayments&bookingId=${encodeURIComponent(body.bookingId||'')}`,
      cancel_url: `${site}/public/confirmation.html?provider=nowpayments&bookingId=${encodeURIComponent(body.bookingId||'')}`
    };
